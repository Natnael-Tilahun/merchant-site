...
export const useEmployees = () => {
  ...
  const getBranchOperators: (id: string) => ApiResult<Employee[]> = async (
    id
  ) => {
    try {
      const { data, pending, error, status } = await fetch<Employee[]>(
  ...
  ) => ApiResult<Employee> = async (employeeData) => {
    try {
      const { data, pending, error, status } = await fetch<Employee>(
        "/api/v1/merchants/operators",
        {
  ...
  const updateEmployee: (
    id: string,
    employeeData: Employee
  ) => ApiResult<Employee> = async (id, employeeData) => {
    try {
      const { data, pending, error, status } = await fetch<Employee>(
        `/api/v1/merchants/operators/${id}`,
        {
  ...
  const resetEmployeePassword: (
    id: string,
    newPassword: string
  ) => ApiResult<Employee | null> = async (id, newPassword) => {
    const employeeData = {
